...
def predict_sentiment_post(short_code="CBgHs1Xjgin", max_comments=10):
    ...
    if not cache:
        cache = joblib.load(open('/app/sentiment_api/ml/cache/predict_param.pickle', 'rb'))
        model = cache['clf']
        tfdf = cache['tfdf']
...
#
#     def get_queryset(self):
#             """Return the authenticated user's comments for all posts"""
#             user = self.request.user
#             post = Post.objects.filter(user=user)
#             return super().get_queryset().filter(post__in=post)
...
class CommentViewSet(viewsets.ModelViewSet):
    ...
    # @action(methods=['get'], detail=False, url_path='comments_posts/(?P<post_id>\d+)', url_name='comments-post')
