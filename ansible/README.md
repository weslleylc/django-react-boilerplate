...
```bash
poetry shell
Spawning shell within /home/user/.cache/pypoetry/virtualenvs/instagramania-ansible-Yf1XQtRW-py3.6 ...
(instagramania-ansible-Yf1XQtRW-py3.6) user@host:.ansible $
```
...
* [Create the bucket for media files](http://docs.aws.amazon.com/AmazonS3/latest/UG/CreatingaBucket.html):
  * Bucket name: instagramania-{ENV} where `ENV` is either `staging` or `production`.
